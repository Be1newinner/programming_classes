    ...
    <main className="min-h-screen bg-black text-white font-sans selection:bg-purple-500/30">
      <div className="max-w-xl mx-auto py-16 px-4">
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-12">
          <button className="flex-1 bg-linear-to-r from-purple-600 to-pink-600 hover:from-purple-500 hover:to-pink-500 text-white font-bold py-3 px-6 rounded-xl transition-all duration-300 transform hover:scale-[1.02] shadow-lg shadow-purple-900/40 active:scale-[0.98]" onClick={addtask}>
            Add Task
          </button>
          ...
          {todolist.map((item, index) => (
            ...
            >
              ...
              <div className="flex-1">
                <p className={`text-lg font-medium ${item.status ? 'text-zinc-500 line-through decoration-zinc-600' : 'text-zinc-200'}`}>
                  {item.title}
                </p>
                <p className="text-xs text-zinc-600 mt-1 font-mono">{item.date}</p>
              </div>

              <div className="flex gap-2 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity duration-200">
                <button className="p-2 rounded-lg hover:bg-zinc-800 text-zinc-400 hover:text-red-400 transition-colors" aria-label="Delete">
                  <img
